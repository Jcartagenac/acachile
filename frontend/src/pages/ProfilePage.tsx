...
const SectionModule: React.FC<SectionModuleProps> = ({ title, description, contact, resources = [], tournaments = [] }) => (
  <div className="bg-white/60 backdrop-blur-soft border border-white/30 rounded-2xl shadow-soft-lg p-6 sm:p-8 space-y-6">
    ...
    {tournaments.length > 0 && (
      <div className="relative overflow-hidden rounded-[32px] border border-neutral-900/10 bg-[linear-gradient(135deg,_rgba(17,24,39,0.98)_0%,_rgba(28,28,31,0.96)_48%,_rgba(120,53,15,0.94)_100%)] p-5 shadow-[0_28px_80px_-28px_rgba(15,23,42,0.75)] sm:p-7">
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          <div className="absolute -left-10 top-0 h-40 w-40 rounded-full bg-orange-400/20 blur-3xl" />
          <div className="absolute right-0 top-12 h-44 w-44 rounded-full bg-primary-400/15 blur-3xl" />
          <div className="absolute bottom-0 left-1/3 h-32 w-32 rounded-full bg-white/10 blur-3xl" />
        </div>

        <div className="relative flex flex-col gap-4 rounded-[28px] border border-white/10 bg-white/[0.04] p-5 backdrop-blur-sm sm:flex-row sm:items-end sm:justify-between sm:p-6">
          <div>
            <span className="inline-flex items-center rounded-full border border-orange-300/30 bg-orange-400/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.24em] text-orange-100">
          ...
          <div className="rounded-2xl border border-white/15 bg-white/10 px-4 py-3 text-sm font-semibold text-white shadow-[0_10px_30px_-18px_rgba(255,255,255,0.8)]">
            {tournaments.length} torneos registrados
          </div>
        </div>

        <div className="relative mt-6 grid gap-5 xl:grid-cols-2">
          {tournaments.map((tournament) => (
            <div
              key={`${tournament.month}-${tournament.name}`}
              className="group rounded-[30px] border border-orange-200/70 bg-[linear-gradient(180deg,_rgba(255,255,255,0.98)_0%,_rgba(255,247,237,0.98)_100%)] p-6 shadow-[0_22px_45px_-26px_rgba(234,88,12,0.45)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_30px_60px_-24px_rgba(234,88,12,0.55)] sm:p-7"
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="max-w-[75%]">
                  <div className="flex flex-wrap gap-2">
                    <span className="rounded-full bg-neutral-950 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.22em] text-white shadow-soft-sm">
                      {tournament.month}
                    </span>
                  ...
                  <h4 className="mt-4 text-2xl font-black leading-tight text-neutral-950 sm:text-[1.7rem]">
                    {tournament.name}
                  </h4>
                </div>
                <div className="rounded-[22px] bg-[linear-gradient(135deg,_#f97316_0%,_#ea580c_100%)] px-4 py-3 text-right text-white shadow-[0_18px_40px_-20px_rgba(249,115,22,0.9)]">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-white/80">Fecha</p>
                  <p className="mt-1 text-base font-black sm:text-lg">{tournament.date}</p>
                </div>
              </div>

              <div className="mt-5 flex flex-wrap gap-2">
                <span className="rounded-full border border-orange-300 bg-white px-3 py-1 text-xs font-semibold text-orange-700 shadow-soft-sm">
                  País: {tournament.country}
                </span>
                <span className="rounded-full border border-neutral-200 bg-neutral-950 px-3 py-1 text-xs font-semibold text-white shadow-soft-sm">
                  Inscripción: {tournament.registrationFee}
                </span>
                <span className="rounded-full border border-neutral-200 bg-white px-3 py-1 text-xs font-semibold text-neutral-700 shadow-soft-sm">
                  Cupos: {tournament.slots}
                </span>
              </div>

              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                <div className="rounded-2xl border border-orange-100 bg-white p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)]">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-neutral-500">País</p>
                  <p className="mt-1 text-sm font-bold text-neutral-900">{tournament.country}</p>
                </div>
                <div className="rounded-2xl border border-orange-100 bg-white p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)]">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-neutral-500">Organización</p>
                  <p className="mt-1 text-sm font-bold text-neutral-900">{tournament.organization}</p>
                </div>
                <div className="rounded-2xl border border-orange-100 bg-white p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)]">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-neutral-500">Valor inscripción</p>
                  <p className="mt-1 text-sm font-bold text-neutral-900">{tournament.registrationFee}</p>
                </div>
                <div className="rounded-2xl border border-orange-100 bg-white p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)]">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-neutral-500">Miembros por equipo</p>
                  <p className="mt-1 text-sm font-bold text-neutral-900">{tournament.teamMembers}</p>
                </div>
                <div className="rounded-2xl border border-orange-100 bg-[linear-gradient(135deg,_rgba(255,237,213,0.85)_0%,_rgba(255,255,255,1)_100%)] p-4 sm:col-span-2">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-neutral-500">Cupos</p>
                  <p className="mt-1 text-sm font-bold text-neutral-900">{tournament.slots}</p>
                </div>
              </div>
            </div>
          ))}
